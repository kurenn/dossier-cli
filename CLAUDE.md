...
would be one diff. The compensating control is §9.2's contract job — see "Not yet built".
...
**The contract job (§9.2) is deferred, not cancelled.** `bin/rails cli:seed_contract` in
the Rails repo already exists and prints a deterministic vault's tokens as JSON. Wiring
a CI job here to consume it needs a cross-repo credential, because `kurenn/dossier` is
private — that decision is open. Until it is made, the `httptest` fakes in this repo are
unarbitrated: they were derived from the documented envelopes, and nothing currently
proves they have not drifted from what the server really sends.
